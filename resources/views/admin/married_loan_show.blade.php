...
<head>
    ...
    <style>
        .product_area .container .col-sm-6 {
            min-width: 100%;
        }
        .product_area .container .col-sm-2{
            min-width: 100%;
        }
        .section-pt-xl{
            padding: 50px 0;
        }
        .container .file-caption .file-caption-name{
            width: 100%;
...
<div id="wrapper" class="wrapper">
    ...
    <!--<input id="id_number" type="file" multiple class="file" data-overwrite-initial="false" name="id_number">-->
    ...
    <!--<div class="product_area section-pt-xl section-pb-xl bg-white">-->
        <!--<div class="container">-->
            <!--<div class="from-group">-->
                ...
                <!--<div class="col-sm-6">-->
                    <!--&lt;!&ndash;<input class="file form-control" placeholder="" name="images[]" type="file" id="img" multiple>&ndash;&gt;-->
...
<script>
    ...
    function init_multiple(id,initialPreview,initialPreviewConfig,order_id)
    {
        ...
        $('#'+id).fileinput({
            language: 'zh',
            uploadUrl: api+'/api/married_upload?name='+id+'&order_id='+order_id,
    ...
    // //ajax 获得资源初始化多图
    function get_images(id,url,order_id)
    {
        $.ajax({
            type:'GET',
            url:url+order_id+'&name='+id,
            success:function(res){
                console.log(res);
                if(res){
                    init_multiple(id,res.images,res.delete,order_id);
                }else{
                    init_multiple(id,[],[],order_id);
                }
    ...
    function getUrlParam(str){
        var reg = new RegExp("(^|&)" + str + "=([^&]*)(&|$)"); //构造一个含有目标参数的正则表达式对象
        var r = window.location.search.substr(1).match(reg);  //匹配目标参数
        if (r != null) return unescape(r[2]); return null; //返回参数值
    }
    var api='http://106.12.16.65';
    var url=api+'/api/show_upload?type=1&order_id=';
    var order_id = getUrlParam('order_id');
    console.log(order_id);
    get_images('id_card',url,order_id);
    get_images('reputation',url,order_id);
    get_images('flowing',url,order_id);
    get_images('household',url,order_id);
    get_images('driver',url,order_id);
    // get_images('divorce',url,28);
    get_images('income',url,order_id);
    get_images('house',url,order_id);

    get_images('g_id_card',url,order_id);
    get_images('g_reputation',url,order_id);
    get_images('g_flowing',url,order_id);
    get_images('g_household',url,order_id);
    get_images('g_driver',url,order_id);
    get_images('g_divorce',url,order_id);
    get_images('g_income',url,order_id);
    get_images('g_house',url,order_id);
